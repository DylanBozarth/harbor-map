...
const MapComponent = ({
  ...
  */
  ...
  const handleLocationClick = (location: LocationInterface) => {
    ...
    setOpenInfoWindowId((prevId) => (prevId === location.id ? null : location.id))
  ...
  return (
    <div className={`harbor-map-container ${className}`} style={{ width, height }}>
      {apiKey ? (
        <APIProvider apiKey={apiKey}>
          ...
          <div className="harbor-map-wrapper">
            {isLoading && <div className="harbor-loading-overlay">Loading map data...</div>}
            <Map
        ...
        <div className="harbor-error-message">Something has gone wrong with our map, please try again later</div>
      )}
    </div>
  )
}
